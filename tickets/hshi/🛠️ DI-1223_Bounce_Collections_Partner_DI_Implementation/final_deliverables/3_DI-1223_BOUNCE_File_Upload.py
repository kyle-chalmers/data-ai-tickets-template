...
# Includes channel suppression flags for Bounce to handle their own filtering
query = '''
select L.LOAN_ID                                                            as ACCOUNT_NUMBER,
       ...
       -- Channel suppression flags for Bounce to handle their own filtering
       case when LCR.SUPPRESS_PHONE = true then true else false end         as SUPPRESS_PHONE,
       case when LCR.SUPPRESS_EMAIL = true then true else false end         as SUPPRESS_EMAIL,
       case when LCR.SUPPRESS_TEXT = true then true else false end          as SUPPRESS_TEXT
...
# MAGIC 
# MAGIC **Job**: DI-1223_BOUNCE_File_Upload
# MAGIC **Purpose**: Daily upload of Bounce collections accounts to SFTP
# MAGIC **File Format**: SIMM-compatible with channel suppression flags (Happy_Money_Bounce_DPD3-119_YYYYMMDD.csv)
...
# MAGIC - Uses same base file format as SIMM for compatibility
# MAGIC - Includes channel suppression flags (SUPPRESS_PHONE, SUPPRESS_EMAIL, SUPPRESS_TEXT)
# MAGIC - Bounce handles their own channel filtering based on these flags
# MAGIC
