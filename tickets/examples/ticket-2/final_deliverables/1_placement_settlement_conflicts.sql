/*
================================================================================
ticket-2: Placement Data Quality Analysis - DebtBuyerA and DebtBuyerB (CORRECTED)
================================================================================

BUSINESS CONTEXT:
Identify charged-off loans marked with DebtBuyerA or DebtBuyerB placement status
that show evidence of continued servicing by FinanceCo through ANY of:
...
- Added future payment transactions from VW_SYSTEM_PAYMENT_TRANSACTION (actual scheduled payments)
...
WITH post_chargeoff_payments AS (
    ...
    ON pt.TRANSACTION_DATE > vl.CHARGEOFFDATE
        ...
        AND VL.PLACEMENT_STATUS IN ('Placed - DebtBuyerA', 'Placed - DebtBuyerB')
    WHERE 1=1
...
SELECT
    ...
    lds.CURRENT_STATUS AS CURRENT_SYSTEM_STATUS,
...
INNER JOIN BUSINESS_INTELLIGENCE.BRIDGE.VW_loan_management_system_CUSTOM_LOAN_SETTINGS_CURRENT lcs
...
WHERE mlt.CHARGEOFFDATE IS NOT NULL
  AND mlt.PLACEMENT_STATUS IN ('Placed - DebtBuyerA', 'Placed - DebtBuyerB')
