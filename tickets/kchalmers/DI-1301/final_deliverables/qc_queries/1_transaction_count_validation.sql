-- QC Query 1: Validate transaction counts in fortress Q3 2025 results
...
loan_details AS (
    SELECT
        vl.loan_id, LOWER(vl.lead_guid) as lead_guid, vl.legacy_loan_id,
        vl.origination_date, vl.loan_amount, vl.term, vl.interest_rate,
        vl.loan_closed_date, vl.charge_off_date, afl.attachment_source
...
fortress_results AS (
    SELECT
        ...
        lp.LOANID as PAYOFF_LOAN_ID,
        LOWER(ld.lead_guid) as PAYOFFUID,
        ld.loan_id as LOANID,
        lp.TRANSACTION_DATE as LOAN_TRANSACTION_DATE,
        lp.APPLY_DATE,
        lp.REVERSAL_DATE,
        lp.LAST_UPDATED as LAST_UPDATED_DATE
...
--1.1: Duplicate Check
SELECT
    'Q3 2025 Fortress Transaction Count Validation' as validation_check,
    COUNT(*) as total_rows,
    COUNT(DISTINCT TRANSACTION_ID) as distinct_transaction_ids,
    COUNT(*) - COUNT(DISTINCT TRANSACTION_ID) as duplicate_transaction_count,
    CASE
        WHEN COUNT(*) = COUNT(DISTINCT TRANSACTION_ID) THEN 'PASS'
        ELSE 'FAIL'
    END as duplicate_check_result
FROM fortress_results;
