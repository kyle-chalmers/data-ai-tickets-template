...
LEFT JOIN UNSUBSCRIBED_RPT C
    ON A.EMAIL_ADDRESS IS NULL
    AND TRIM(LOWER(B.EMAIL)) = C.EMAIL_NORMALIZED
WHERE (A.EMAIL_ADDRESS IS NOT NULL OR C.EMAIL_NORMALIZED IS NOT NULL)
QUALIFY ROW_NUMBER() OVER (PARTITION BY B.PAYOFFUID ORDER BY COALESCE(A.CREATED_DATE, C.DATEUNSUBSCRIBED) DESC) = 1
