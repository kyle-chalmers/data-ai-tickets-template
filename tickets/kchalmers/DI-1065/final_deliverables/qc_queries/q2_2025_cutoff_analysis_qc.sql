-- DI-1065: Q2 2025 Cutoff Analysis - Compare transaction counts before and after June 30, 2025
...
transaction_summary AS (
    SELECT 
        ...
        COUNT(DISTINCT ld.lead_guid) as total_loans,
        COUNT(DISTINCT CASE WHEN lp.PAYMENT_ID IS NOT NULL AND lp.TRANSACTION_DATE <= '2025-06-30' THEN ld.lead_guid END) as loans_with_q2_transactions,
        COUNT(DISTINCT CASE WHEN lp.PAYMENT_ID IS NOT NULL AND lp.TRANSACTION_DATE > '2025-06-30' THEN ld.lead_guid END) as loans_with_only_q3_transactions,
        COUNT(CASE WHEN lp.TRANSACTION_DATE <= '2025-06-30' THEN 1 END) as q2_transaction_count,
        COUNT(CASE WHEN lp.TRANSACTION_DATE > '2025-06-30' THEN 1 END) as q3_transaction_count,
        COUNT(*) as total_transaction_count,
        MIN(lp.TRANSACTION_DATE) as earliest_transaction,
        MAX(CASE WHEN lp.TRANSACTION_DATE <= '2025-06-30' THEN lp.TRANSACTION_DATE END) as latest_q2_transaction,
...
SELECT 
    attachment_source,
    total_loans,
    loans_with_q2_transactions,
    ROUND(loans_with_q2_transactions * 100.0 / total_loans, 1) as q2_coverage_pct,
    loans_with_only_q3_transactions,
    q2_transaction_count,
    q3_transaction_count,
    total_transaction_count,
    earliest_transaction,
    latest_q2_transaction,
    latest_transaction,
    CONCAT('Excluded ', q3_transaction_count, ' transactions after June 30, 2025') as cutoff_impact
FROM transaction_summary
ORDER BY attachment_source;

-- Additional detail: Show which loans have July transactions that will be excluded
WITH july_transactions AS (
    SELECT 
        ...
        SUM(lp.TRANSACTION_AMOUNT) as july_transaction_total
    FROM loan_details ld
    JOIN BUSINESS_INTELLIGENCE.ANALYTICS.VW_LP_PAYMENT_TRANSACTION lp 
        ON UPPER(lp.loan_id::TEXT) = UPPER(ld.loan_id::TEXT)
    WHERE lp.IS_MIGRATED = 0
      AND lp.TRANSACTION_DATE > '2025-06-30'
    GROUP BY 1, 2, 3
)

SELECT 
    attachment_source,
    COUNT(*) as loans_with_july_transactions,
    SUM(july_transaction_count) as total_july_transactions_excluded,
    ROUND(SUM(july_transaction_total), 2) as total_july_amount_excluded
