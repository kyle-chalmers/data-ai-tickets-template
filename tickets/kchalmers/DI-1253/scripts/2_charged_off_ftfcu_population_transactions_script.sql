...
OPTIMIZED Transaction history query - TRANSACTION REPORT

OPTIMIZATIONS MADE:
1. Pre-filter all data sources to only loans in the population table
2. Eliminated redundant CTEs and intermediate calculations
3. Simplified payment transaction logic
4. Optimized window functions and qualify statements
5. Reduced column selection in intermediate steps
6. Combined adjustment calculations more efficiently
...
CREATE OR REPLACE TABLE BUSINESS_INTELLIGENCE_DEV.CRON_STORE.CHARGE_OFF_FTFCU_ALLOCATED_POPULATION_TRANSACTIONS COPY GRANTS AS
...
--charged_off_ftfcu_allocated_population_transactions
SELECT * FROM BUSINESS_INTELLIGENCE_DEV.CRON_STORE.CHARGE_OFF_FTFCU_ALLOCATED_POPULATION_TRANSACTIONS
order by LOAN_TRANSACTION_DATE desc
;

-- QC: Check for duplicate TRANSACTION_ID per LOANID
SELECT COUNT(*) AS ROW_COUNT, A.TRANSACTION_ID, A.LOANID
FROM BUSINESS_INTELLIGENCE_DEV.CRON_STORE.CHARGE_OFF_FTFCU_ALLOCATED_POPULATION_TRANSACTIONS A
GROUP BY A.TRANSACTION_ID, A.LOANID
HAVING COUNT(*) > 1
ORDER BY 1 DESC;
...
SELECT 'POPULATION' as source, COUNT(*) AS ROW_COUNT, COUNT(DISTINCT LOANID) AS DISTINCT_LOANID_COUNT
FROM BUSINESS_INTELLIGENCE_DEV.CRON_STORE.CHARGE_OFF_FTFCU_ALLOCATED_POPULATION_TRANSACTIONS
UNION ALL
SELECT 'TRANSACTIONS' as source, COUNT(*) AS ROW_COUNT, COUNT(DISTINCT LOANID) AS DISTINCT_LOANID_COUNT
FROM BUSINESS_INTELLIGENCE_DEV.CRON_STORE.CHARGE_OFF_FTFCU_ALLOCATED_POPULATION_TRANSACTIONS;