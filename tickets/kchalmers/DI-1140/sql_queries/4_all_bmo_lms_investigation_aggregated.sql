...
SELECT 
    ...
    COUNT(CASE WHEN le.CREATED >= DATEADD('day', -$DAYS_THRESHOLD, CURRENT_DATE()) THEN 1 END) as RECENT_LOANS,
    ...
    -- GIACT fields removed due to data quality issues
    ...
    ROUND(AVG(CAST(LOS.REQUESTED_LOAN_AMOUNT AS FLOAT)), 2) as AVG_REQUESTED_AMOUNT,
    
    -- === ENHANCED PARTNER AND STATE ANALYSIS ===
    LISTAGG(DISTINCT LOS.CAPITAL_PARTNER, ', ') as CAPITAL_PARTNERS_USED,
    COUNT(DISTINCT LOS.CAPITAL_PARTNER) as UNIQUE_CAPITAL_PARTNERS,
    LISTAGG(DISTINCT LOS.HOME_ADDRESS_STATE, ', ') as APPLICANT_STATES,
    COUNT(DISTINCT LOS.HOME_ADDRESS_STATE) as UNIQUE_STATES,
    
    -- === ACQUISITION CHANNEL ANALYSIS ===
    LISTAGG(DISTINCT LOS.UTM_SOURCE, ', ') as ACQUISITION_SOURCES,
    COUNT(DISTINCT LOS.UTM_SOURCE) as UNIQUE_ACQUISITION_SOURCES
...
-- Join back to LOS application data via LEAD_GUID
