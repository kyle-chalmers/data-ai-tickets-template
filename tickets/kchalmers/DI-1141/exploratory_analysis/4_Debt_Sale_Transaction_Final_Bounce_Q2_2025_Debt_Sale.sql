...
Transaction history query - TRANSACTION REPORT
...
NOTE: Run the population query FIRST, then run this transaction query
This query inner joins to the created population table
*/

USE WAREHOUSE BUSINESS_INTELLIGENCE_LARGE;
USE ROLE BUSINESS_INTELLIGENCE_PII;

CREATE OR REPLACE TABLE BUSINESS_INTELLIGENCE_DEV.CRON_STORE.BOUNCE_DEBT_SALE_Q2_2025_SALE_TRANSACTIONS AS
...
---------- 2 columns need to be fixed in this cte, with the listed above fields to be found
    SELECT A.TRANSACTION_ID
        ...
        ,a.LOAN_CLEARED
    FROM
        BUSINESS_INTELLIGENCE_DEV.CRON_STORE.BOUNCE_DEBT_SALE_Q2_2025_SALE clc
...
--------------- NEW -----------------------------
    ...
    FROM
        ...
        INNER JOIN BUSINESS_INTELLIGENCE_DEV.CRON_STORE.BOUNCE_DEBT_SALE_Q2_2025_SALE clc
...
-- View the results
SELECT *
FROM BUSINESS_INTELLIGENCE_DEV.CRON_STORE.BOUNCE_DEBT_SALE_Q2_2025_SALE_TRANSACTIONS;

-- QC: Check for duplicate TRANSACTION_ID per LOANID
SELECT COUNT(*) AS ROW_COUNT,
A.TRANSACTION_ID,
A.LOANID
FROM BUSINESS_INTELLIGENCE_DEV.CRON_STORE.BOUNCE_DEBT_SALE_Q2_2025_SALE_TRANSACTIONS A
GROUP BY ALL
HAVING ROW_COUNT > 1
ORDER BY 1 DESC;
...
-- QC: Compare with population table
SELECT 'POPULATION' as source, COUNT(*) AS ROW_COUNT,
COUNT(DISTINCT LOANID) AS DISTINCT_LOANID_COUNT
FROM BUSINESS_INTELLIGENCE_DEV.CRON_STORE.BOUNCE_DEBT_SALE_Q2_2025_SALE
UNION ALL
SELECT 'TRANSACTIONS' as source, COUNT(*) AS ROW_COUNT,
COUNT(DISTINCT LOANID) AS DISTINCT_LOANID_COUNT
FROM BUSINESS_INTELLIGENCE_DEV.CRON_STORE.BOUNCE_DEBT_SALE_Q2_2025_SALE_TRANSACTIONS;