/* QC VALIDATION - Record Counts and Statistics
 * Provides aggregate counts for validation
 */

USE WAREHOUSE BUSINESS_INTELLIGENCE_LARGE;
USE DATABASE BUSINESS_INTELLIGENCE;

-- 2.1: Count validation with INNER JOINs
...
-- 2.2: Count validation with LEFT JOINs (to check for data loss)
WITH UNSUBSCRIBED_LIST AS (
    SELECT
        A.SUBSCRIBER_KEY,
        A.CREATED_DATE,
...
,FINAL_QUERY AS (
    SELECT DISTINCT
        ...
        C.STATE
    FROM ANALYTICS.VW_LOAN A
    LEFT JOIN BRIDGE.VW_LOAN_STATUS_ARCHIVE_CURRENT B
        ON A.LOAN_ID::VARCHAR = B.LOAN_ID::VARCHAR
        AND B.DATE = CURRENT_DATE
        AND B.DAYS_PAST_DUE < 3
    LEFT JOIN ANALYTICS_PII.VW_MEMBER_PII C
        ON A.MEMBER_ID = C.MEMBER_ID
        AND C.MEMBER_PII_END_DATE IS NULL
    LEFT JOIN BRIDGE.VW_LOAN_CUSTOMER_CURRENT E
        ON A.APPLICATION_ID::VARCHAR = E.LOAN_ID::VARCHAR
        AND E.SCHEMA_NAME = CONFIG.LOS_SCHEMA()
    INNER JOIN ANALYTICS.VW_LOAN_CONTACT_RULES F
        ON A.LOAN_ID::VARCHAR = F.LOAN_ID::VARCHAR
