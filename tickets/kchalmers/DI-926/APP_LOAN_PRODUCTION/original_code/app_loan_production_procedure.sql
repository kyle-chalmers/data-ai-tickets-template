...
on a.app_id = c.app_id
          and c.applied_min_ts is not null
...
,case when c.loan_docs_completed_max_ts is not null then e.loan_amt else null end::numeric(10,2) as loan_amt
,case when c.loan_docs_completed_max_ts is not null then e.orig_fee_amt else null end::numeric(10,2) as orig_fee_amt                    
,case when c.loan_docs_completed_max_ts is not null and e.loan_amt > 0 then e.orig_fee_amt / e.loan_amt else null end::numeric(10,4) as orig_fee_prct 
,case when c.loan_docs_completed_max_ts is not null then e.prem_fee_amt else null end::numeric(10,2) as prem_fee_amt                     
,case when e.loan_amt > 0 then e.prem_fee_amt / e.loan_amt else null end::numeric(10,4) as prem_fee_prct 
,case when c.loan_docs_completed_max_ts is not null then e.term else null end::integer as term                              
,case when c.loan_docs_completed_max_ts is not null then e.apr_prct else null end::numeric(10,4) as apr_prct                    
,case when c.loan_docs_completed_max_ts is not null then e.int_rate_prct else null end::numeric(10,4) as int_rate_prct               
,case when c.loan_docs_completed_max_ts is not null then e.monthly_payment_amt else null end::numeric(10,2) as monthly_payment_amt        
...
update business_intelligence.bridge.app_loan_production a
set
guid_dupe_yn = case     when b.guid is not null and a.app_id = b.esign_app_id then ''N''
...
from 
        ...
        app_id 
        from business_intelligence.bridge.app_loan_production
        where declined_ts > esign_ts or withdrawn_ts > esign_ts
        ) b
where a.app_id = b.app_id
;
