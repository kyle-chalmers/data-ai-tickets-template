CREATE OR REPLACE PROCEDURE BUSINESS_INTELLIGENCE.BRIDGE.APP_SYSTEM_NOTE_ENTITY()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS '
...
delete from business_intelligence.bridge.app_system_note_entity where created_ts::date >= (select max(created_ts::date) as max_dt from business_intelligence.bridge.app_system_note_entity);

insert into business_intelligence.bridge.app_system_note_entity
--create or replace table business_intelligence.bridge.app_system_note_entity as 
select
a.record_id
,a.app_id
,a.created_ts
...
,null::varchar(255) as note_new_value_label       
...
,a.deleted
,a.is_hard_deleted
,trim(replace(object_keys(parse_json(a.note_data):"PortfoliosAdded":"newValue")[0],''"'',''''))::STRING as portfolios_added
,trim(replace(object_keys(parse_json(a.note_data):"PortfoliosRemoved":"newValue")[0],''"'',''''))::STRING as portfolios_removed
,null::varchar(55) as portfolios_added_category 
,null::varchar(55) as portfolios_added_label 
from
            (select 
            id as record_id
            ,entity_id as app_id
            ...
            where schema_name = CONFIG.LOS_SCHEMA()                         --value = 5203309_P                                                                
                ...
                and convert_timezone(''UTC'',''America/Los_Angeles'',created) >= (select max(created_ts) as max_dt from business_intelligence.bridge.app_system_note_entity)
                --and entity_id = 710885
                --order by 2,3
    ) a
...
update business_intelligence.bridge.app_system_note_entity a
set
note_new_value_label = b.label
from business_intelligence.cron_store.loanpro_los_lms_labels b
where a.note_title_detail = b.group_name
    and a.note_new_value = b.id
    and b.schema_name = ''LOS'' 
    and a.created_ts::date >= current_date - 1
;


update business_intelligence.bridge.app_system_note_entity a
set 
portfolios_added_category = portfolio_category
,portfolios_added_label = portfolio_title
from 
            (select 
            a.id as portfolio_id
            ,trim(a.title)::varchar(255) as portfolio_title
...
where try_to_number(a.portfolios_added) = b.portfolio_id
    and a.created_ts::date >= current_date - 1
;
