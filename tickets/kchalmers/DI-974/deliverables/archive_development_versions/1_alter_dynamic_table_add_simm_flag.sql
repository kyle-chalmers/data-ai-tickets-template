-- DI-974: ALTER DSH_GR_DAILY_ROLL_TRANSITION Dynamic Table to Add SIMM Placement Flag
-- This modifies the existing dynamic table to include SIMM placement information

-- DI-974: Modify DSH_GR_DAILY_ROLL_TRANSITION Dynamic Table to Add SIMM Placement Flags
-- NOTE: Dynamic tables require CREATE OR REPLACE - no separate ALTER statements needed
-- The new columns (CURRENT_SIMM_PLACEMENT_FLAG, HISTORICAL_SIMM_PLACEMENT_FLAG, FIRST_SIMM_DATE) 
-- are automatically added when the dynamic table is recreated with the updated query
...
,daily_loan_tape as (SELECT A.LOANID,
                           ...
                           FROM BUSINESS_INTELLIGENCE.DATA_STORE.MVW_LOAN_TAPE_DAILY_HISTORY A
                                    LEFT JOIN BUSINESS_INTELLIGENCE.CRON_STORE.TEMP_DI_652 B
                                              ON A.LOANID = B.LOANID AND DATE(A.ASOFDATE) = DATE(B.AS_OF_DATE))
-- Add SIMM placement data for historical tracking
,simm_placements AS (
    SELECT 
        PAYOFFUID,
...
from (
    ...
    --     dlt.PAYMENTANNIVERSARYDATE = day(dlt.ASOFDATE) as payment_anniversary_flag, -- what is this?
        ...
        -- Add current SIMM placement flag - exact date match for daily activity
        CASE 
        ...
        -- Add historical SIMM placement flag - ever been placed with SIMM
        CASE 
    ...
    -- Join current SIMM placement data (exact date match for daily activity)
    ...
    -- Join historical SIMM placement data (cumulative status)
