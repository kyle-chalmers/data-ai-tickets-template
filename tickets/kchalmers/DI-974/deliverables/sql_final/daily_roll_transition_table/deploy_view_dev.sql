-- DI-974: DEV VERSION - Daily Roll Transition View (since we can't create dynamic table in DEV)
-- DEPLOYED TO: BUSINESS_INTELLIGENCE_DEV.CRON_STORE (Development Testing)

CREATE OR REPLACE DYNAMIC TABLE BUSINESS_INTELLIGENCE.REPORTING.DSH_GR_DAILY_ROLL_TRANSITION
target_lag = '12 hours' refresh_mode = AUTO initialize = ON_CREATE warehouse = BUSINESS_INTELLIGENCE
AS
