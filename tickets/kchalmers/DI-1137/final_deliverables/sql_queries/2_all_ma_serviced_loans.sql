...
-- Main query for all Massachusetts loans
WITH ALL_MA_LOANS AS (
    SELECT 
...
-- Final output with all required fields (get most recent status per loan)
SELECT DISTINCT
    APPLICATION_DATE,
...
WITH ALL_MA_LOANS_SUMMARY AS (
    SELECT 
        ...
        B.INTEREST_RATE,
        B.APR,
        'Status TBD' AS CURRENT_STATUS
    FROM BUSINESS_INTELLIGENCE.ANALYTICS.VW_LOAN A
    INNER JOIN BUSINESS_INTELLIGENCE.ANALYTICS.VW_APPLICATION B
        ON A.APPLICATION_ID = B.APPLICATION_ID
    WHERE UPPER(A.APPLICANT_RESIDENCE_STATE) = $STATE_FILTER
)
SELECT 
    'All MA Serviced Loans' AS QUERY_TYPE,
